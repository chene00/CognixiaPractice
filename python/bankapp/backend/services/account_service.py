...
class AccountService():
    ...
    def createAccount(self, custID : int, newAcc : AccountCreate) -> Account:
        if newAcc.balance < 0:
            raise HTTPException(status_code=400, detail="Balance can't be negative")

        customer = self.cust_repo.get_id_customer(custID)
        if not customer:
            raise HTTPException(status_code=404, detail=f"Customer {custID} not found")
        
        created_account = self.repo.create_account(custID, newAcc)

        customer.accounts.append(created_account)
